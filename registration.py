...
def login(): 
    ...
    print(hexd_password)
    actual_password = firebase.get('/',username)
    if(actual_password != None):
        if(hexd_password == actual_password):
            print("success")
            label_thiny["text"] = "Success!"
        else:
            print("wrong password mah ded")
            label_thiny["text"] = "Incorrect Password"
    else: 
        print("error, go fix it or smthing idk")
        label_thiny["text"] = "No account found with entered username"
...
def login_window():
    ...
    label_thiny = Label(login_window, font = 'arial 18 bold', bg="azure")
    label_thiny.place(relx=0.5,rely=0.75,anchor=CENTER)
